...
\begin{center}
	...
	% ice -- blue
	\fill [opacity=0.4,blue] (lowleft) -- (lowright) -- (upright) -- (upleft) -- (lowleft);
	% no ice -- red
	\fill [opacity=0.4,red] (arcstart) -- ($(arcstart)+({3/8*\xmax},0)$) --
    ...
    \node[empty node] at ($(arcstart)+({-0.1*\xmax},0)$) {$(\frac{5L}{8},\frac{3L}{4})$};
    \node[empty node] at ($(arcend)+({-0.1*\xmax},0)$) {$(\frac{5L}{8},\frac{L}{4})$};
